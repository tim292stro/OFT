...
There are some options about 7-zip and ffmpeg when they are not discoverable in the PATH variable which I find are annoying to keep typing out, so this code will create a persistence file (JSON) in the OFT.py script's working directory so it can just use it next call without having to type it out.  It's nothing crazy or risky, just some key:value pairs in human readable form.  I got tired of walking up to a machine that didn't have the environment setup well for this to work without a lot of fidgeting, so I brute forced it.  On that topic, same thing (attempt-auto-fix) with package dependancies.  You're welcome I guess.
